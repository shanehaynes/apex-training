...
- **Prompt caching** sets three `ephemeral` breakpoints (last tool schema, system block, final message block) so each turn re-reads tools, system prompt, and the conversation prefix at cache-read pricing. The stable half of the chat prompt (role, safety, doctrine index, library, rules) is the `system` block under a 1-hour entry; the live half (schedule, meals, block, physiology) is injected per request and never stored in the thread.
- **The coach reads before it speaks.** In chat mode the tool list also carries the read tools (the MCP query set plus session summaries, reviews and history search) and `read_doctrine`. A response that asks only for those is answered server-side — up to five rounds inside one user turn, each executed in parallel, the assistant message echoed back thinking blocks and all, the doctrine handed over as a citable document — and narrated on the wire as `tool_read` / `tool_read_result` events so the sidebar shows "Checked: …" chips above the reply and the stored thread replays as a valid transcript. Write tools keep the confirm card; a response that mixes reads and writes runs the reads, surfaces the writes, and the client's `tool_result` message carries both.
...
| `RESEND_API_KEY` | server only | Resend SMTP for review emails, from `reviews@apex-training.app` ([API keys](https://resend.com/api-keys); Sending access, that domain only) |
