...
// Comments are stripped first: a comment that mentions a token by name
// ("contrast against --bg-surface: ...") otherwise parses as a declaration.
...
// A token may alias another (`--stream-mark: var(--positive);`) so the CSS
// states the relationship instead of repeating a hex. Resolved here, with a
// cycle guard, because Swift has no `var()`.
const need = (name, seen = []) => {
  const v = vars.get(name);
  if (v === undefined) fail(`--${name} is missing from src/styles/tokens.css`);
  const alias = /^var\(\s*--([\w-]+)\s*\)$/.exec(v);
  if (!alias) return v;
  if (seen.includes(name)) fail(`--${name} aliases itself: ${[...seen, name].join(' -> ')}`);
  return need(alias[1], [...seen, name]);
};
const hex = (name) => {
  const v = need(name);
...
// A colour token is either an opaque hex or an `rgba()` — `--border-subtle` is
// the ink at 13%, because a translucent hairline reads as a line where a solid
// grey reads as a box. Both land as one Swift expression.
const colorExpr = (name) => {
  const v = need(name);
  if (/^#[0-9a-fA-F]{6}$/.test(v)) return `Color(hex: 0x${hex(name)})`;
  const { hex: h, alpha } = rgba(v, `--${name}`);
  return `Color(hex: 0x${h}).opacity(${alpha})`;
};
...
// The promoted signals, in the order design-spec §1 lists them. The Swift name
// is `ApexPalette`'s, so the forwarding in Palette.swift stays a rename-free read.
const SIGNALS = [
  { key: 'border-strong', token: 'border-strong' },
  { key: 'positive', token: 'positive' },
  { key: 'danger', token: 'danger' },
  { key: 'danger-text', token: 'danger-text' },
  { key: 'destructive', token: 'destructive' },
  { key: 'user-bubble', token: 'user-bubble' },
  { key: 'user-bubble-border', token: 'user-bubble-border' },
  { key: 'stream-mark', token: 'stream-mark' },
  { key: 'attainment-met', token: 'attainment-met' },
  { key: 'attainment-close', token: 'attainment-close' },
  { key: 'attainment-under', token: 'attainment-under' },
];
...
public enum ApexColor {
    ...
    public static let borderSubtle = ${colorExpr('border-subtle')}
...
${SIGNALS.map((s) => `    public static let ${swiftKey(s.key)} = ${colorExpr(s.token)}`).join('\n')}
}
