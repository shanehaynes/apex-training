...
import { createEvent, deleteEvent, updateEvent, type EventMutationLogEntry } from '../services/events.js';
import { sendFailure } from '../services/result.js';
import type { WorkoutEventRow } from '../../../src/lib/db/types.js';

// HTTP door onto api/_lib/services/events.ts: auth, throttles and body
// parsing here; the mutations themselves are shared with the coach's
// server-side tool executors (W5b).
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'POST') {
    ...
    const result = await createEvent(supabase, userId, row as Record<string, unknown>, triggeredBy);
  ...
  if (req.method === 'PATCH') {
    const body = req.body as { fields?: Partial<WorkoutEventRow>; log?: EventMutationLogEntry } | undefined;
    ...
    const result = await updateEvent(supabase, userId, id, body.fields as Record<string, unknown>, body.log);
    if (!result.ok) return sendFailure(res, result);
  ...
  if (req.method === 'DELETE') {
    const body = req.body as { log?: EventMutationLogEntry } | undefined;
    if (body?.log?.triggered_by !== 'user' && !(await enforceAiMutationCap(supabase, res, userId))) return;

    const result = await deleteEvent(supabase, userId, id, body?.log);
    if (!result.ok) return sendFailure(res, result);
    res.status(200).json({ ok: true });
    return;
  }
